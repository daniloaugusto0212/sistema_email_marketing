<?php
    ...
    if ($sql->rowCount() == 0) {
        Painel::alert('erro', 'O imóvel que você está tentando editar não existe!');
        die();
    }
    ...
    if (isset($_GET['deletarImagem'])) {
        ...
        $pegaImagens->execute();
        $pegaImagens = $pegaImagens->fetchAll();
    }elseif (isset($_GET['deletarImovel'])) {
        ...
        Painel::alertJS('O imóvel foi excluído com sucesso!');
        Painel::redirect(INCLUDE_PATH_PAINEL.'listar-empreendimentos');
    }

?>
        <div class="box-content">
            <h2><i class="fas fa-user-edit"></i> Editando Imóvel: <?php echo $id;?> </h2>    
            <div class="card-title"><i class="fas fa-info-circle"></i> Informações do imóvel:</div>
            <form method="post" action="<?php echo INCLUDE_PATH_PAINEL ?>editar-produto?id=<?php echo $id; ?>"  enctype="multipart/form-data">
                
            <div class="form-group">
                <label>Nome do Imóvel:</label>
                <input disabled="" type="text" name="nome" value="<?php echo $infoImovel['nome'];?>">
            </div><!--form-group-->

            <div class="form-group">
                <label>Preço do Imóvel:</label>
                <input disabled="" type="text" name="nome" value="<?php echo $infoImovel['preco'];?>">
            </div><!--form-group-->

            <div class="form-group">
                <label>Área do Imóvel:</label>
                <input disabled="" type="text" name="nome" value="<?php echo $infoImovel['area'];?>">
            </div><!--form-group-->
